...
from keras.layers import Dense, Input, Flatten, Merge, Bidirectional, TimeDistributed
...
reviews = tokenize.sent_tokenize(docs)
...
# reviews = tokenizer.texts_to_sequences(docs)
...
data = np.zeros((len(docs), MAX_SENTS, MAX_SENT_LENGTH), dtype='int32')
...
l_lstm_sent = Bidirectional(GRU(100, return_sequences=True))(encoder)
...
model.save('cs_classify_biLSTM_hi_att.h5')
...
my_model = load_model('cs_classify_biLSTM_hi_att.h5', custom_objects={'AttLayer':AttLayer})
